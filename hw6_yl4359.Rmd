...
I used baby's sex and length at birth(include their interaction), gestational age in weeks, mother’s weight at delivery and family monthly income(main effects only) as predictors. And constructed a multiple linear regression model. I think these factors will infludence the birth weight of baby. 
...
From the violin plot, we could see that the third model is better, because it has least prediction error. 

# Problem 2

## import weather_df dataset
...
The 95% confidence interval for r^2 is (`r round(quantile_r[[1]], digits = 3)`, `r round(quantile_r[[2]], digits = 3)`).
...
The distribution of estimate log(&beta;<sub>0</sub>*&beta;<sub>1</sub>) is approximately normal. 

The 95% confidence interval for log(&beta;<sub>0</sub>*&beta;<sub>1</sub>) is (`r round(quantile_beta[[1]],digits = 3)`, `r round(quantile_beta[[2]], digits = 3)`).
